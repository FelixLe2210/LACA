...
const asyncHandler = require('../utils/asyncHandler'); // Tận dụng tiện ích có sẵn của bạn
...
const createFeedback = asyncHandler(async (req, res) => {
  ...
  // 2. THÊM HÀM NÀY: Xử lý Báo lỗi (Report Issue)
const createReport = asyncHandler(async (req, res) => {
  ...
  const newReport = await Feedback.create({
    content,
    type: 'report', // Đánh dấu đây là báo lỗi
    userId: userId || null,
    status: 'new'
  });
